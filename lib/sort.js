 ...
 * Stable - NO
 */
export const shellSort = (arr, type = st.ASC) => {
        ...
        for (let i = gap; i < length; i++) {
            ...
            for (j = i; j >= gap &&
                ((type === st.ASC && arr[j - gap] > temp) ||
                    (type === st.DESC && arr[j - gap] < temp)); j -= gap) {
                arr[j] = arr[j - gap];
            }
            arr[j] = temp;
        }
 ...
 * * Space Complexity - O(1)
 */
export const heapSort = (arr, type = st.ASC) => {
    const length = len(arr);
    for (let i = floorDiv(length) - 1; i >= 0; i--)
        heapify(arr, length, i, type);
...
export const mSort = (arr, l, r, type = st.ASC) => {
    if (l >= r) {
        return;
    }
...
export const mergeSort = (arr, type = st.ASC) => {
    const left = 0;
    const right = len(arr) - 1;
    mSort(arr, left, right, type);
    return arr;
};
export const qSort = (arr, low, high, type = st.ASC) => {
    if (low < high) {
...
export const quickSort = (arr, type = st.ASC) => {
    const low = 0;
    const high = arr.length - 1;
    qSort(arr, low, high, type);
    return arr;
};
export const insertionSort = (arr, type = st.ASC) => {
    const n = len(arr);
    let i, j, key;
    for (i = 1; i < n; i++) {
        key = arr[i];
...
export const countSort = (arr, n, exp) => {
...
export const radixSort = (arr, type = st.ASC) => {
    const n = len(arr);
    let m = getMax(arr, n);
...
export const bSort = (arr, n, type) => {
    if (n <= 1)
        return;
    for (let i = 0; i < n - 1; i++) {
        if ((type === st.ASC && arr[i] > arr[i + 1]) ||
...
export const bubbleSort = (arr, type = st.ASC) => {
    const n = len(arr);
    bSort(arr, n, type);
    return arr;
};
export const countingSort = (arr, type = st.ASC) => {
    const n = len(arr);
