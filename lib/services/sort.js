...
export const heapify = (arr, n, i, type = st.ASC) => {
    let largest = i;
    const left = 2 * i + 1;
    const right = 2 * i + 2;
    if (left < n &&
...
export const merge = (arr, left, mid, right, type = st.ASC) => {
    const n1 = mid - left + 1;
    const n2 = right - mid;
    ...
    let k = left;
    while (i < n1 && j < n2) {
        if ((type === st.ASC && tempArr1[i] <= tempArr2[j]) ||
...
export const partition = (arr, low, high, type = st.ASC) => {
    const pivot = arr[high];
    let i = low - 1;
    for (let j = low; j <= high - 1; j++) {
        if ((type === st.ASC && arr[j] < pivot) ||
